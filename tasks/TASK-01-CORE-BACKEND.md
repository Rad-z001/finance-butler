...
- [ ] DI composition root (`container.ts`), typed domain errors + error-handler middleware

### 2. LINE plumbing
- [x] Webhook route: raw-body HMAC signature validation → 200 fast → enqueue
- [ ] BullMQ `line-events` queue + worker, Redis dedupe + `WebhookEvent` unique
- [ ] Follow event → auto-register user (profile fetch, timezone Asia/Bangkok, default Cash account)
- [ ] Reply/push wrappers over `@line/bot-sdk` behind `ILineMessenger` port

### 3. Thai NLP (the heart)
- [ ] `ThaiRuleParser`: amount extraction (`80`, `1,200`, `1.2k`, `80บาท`), Thai date resolver (วันนี้/เมื่อวาน/พรุ่งนี้/วันจันทร์/15 ก.ค./1/7/เดือนที่แล้ว), keyword→category map, merchant capture
- [ ] `CategoryCorrection` lookup (user's past corrections win over everything)
- [ ] Claude fallback: tool-use forced JSON → `ParsedIntent` (shared discriminated union); Haiku for classification, Sonnet for ambiguous parses
- [ ] Command router: สรุปวันนี้/สัปดาห์นี้/เดือนนี้/ปีนี้, ลบ/แก้ล่าสุด, search phrases
- [ ] Conversation state machine in Redis (pending confirmations, TTL 10 min)

### 4. Money services (all inside `prisma.$transaction`, all audited)
- [ ] TransactionService: create/edit/delete(soft)/restore, shortRef allocation, balance updates
- [ ] AccountService: CRUD + transfers; CategoryService; SearchService (Thai keyword + filters)
- [ ] StatsService: summaries, rankings, trends, year compare, cashflow (SQL aggregates + Redis cache)
